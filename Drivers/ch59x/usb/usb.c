...
UINT8 usb_send(const char *fmt, ...)
{
  ...
  va_list args;
  va_start(args, fmt);
  send_len = (UINT16)vsnprintf(buffer, USB_TX_BUFFER_SIZE, fmt, args);
  va_end(args);

  /* Clamp length to buffer size */
  if (send_len > USB_TX_BUFFER_SIZE)
    send_len = USB_TX_BUFFER_SIZE;

  /* 厂商模式处理 */
  if(usb_work_mode == USB_VENDOR_MODE)
  {
    memcpy(&Ep2Buffer[MAX_PACKET_SIZE], buffer, send_len);
  ...
  {
    memcpy(&Ep1Buffer[MAX_PACKET_SIZE], buffer, send_len);
